...
import br.pet.admin.service.RoleService;
import br.pet.admin.service.UserService;
import br.pet.util.mail.sender.Mail;
...
public class UserController {
	...
	public String doUserRegister(Model model, @RequestParam String name, @RequestParam String email,
		...
		try {
			...
			// CentrodeCustoContabil centrodeCusto =
			// centrodeCustoContabilService.findOne(idCentroDeCusto);
			user.setName(name);
			user.setEmail(email);
			...
			user.setRoles(roles);
			user.setDtInclusao(anoInclusao);
			userService.save(user);
		} catch (Exception e) {
		...
		if (user != null) {
			if (user.getToken().equals(token)) {
				if (password1.equals(password2)) {
					...
					return "redirect:/user-password-recovery.html?dp=true";
				}
			} else {
				return "redirect:/user-password-recovery.html?it=true";
			}
		} else {
			return "redirect:/user-password-recovery.html?unf=true";
		}
		return "redirect:/login.html?pref=true";
	}
			...
			try {
				if (user.getEmail() != null) {
					...
					if (to.equals(email)) {
						...
						userService.save(user);
						return "redirect:/user-password-recovery.html?es=true";

					} else {
						System.out.println("Email informado e diferente do cadastrado");
						return "redirect:/user-password-recovery.html?ed=true";
					}
				}
			} catch (Exception e) {
				System.out.println("Exception e: " + e);
				return "redirect:/user-password-recovery.html?enr=true";
			}
		} catch (Exception e) {
